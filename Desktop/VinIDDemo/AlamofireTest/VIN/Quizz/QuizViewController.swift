...
class QuizViewController: UIViewController {
    ...
    @objc func endedFunc(){
        ...
        let action = UIAlertAction(title: "Đồng ý", style: .default) { [weak self] (_) in
            guard let strongSelf = self else { return }
            strongSelf.answers?.spenttime = strongSelf.dem
            strongSelf.tableView.reloadData()
    ...
    func getQuizz(_ id: Int){
        ...
        Alamofire.request(url, method: .get, parameters: nil, encoding: URLEncoding.default, headers: headers).responseObject { [weak self] (response: DataResponse<APIQuizz>) in
            ...
            strongSelf.tableView.reloadData()
            //            strongSelf.listAnswer = res.questionanswers
        }
    }
    ...
    func nopBai(ans: APIPostAnswers){
          ...
          let params = [
              "id" : ans.id ?? 0,
              "spent_time": ans.spenttime ?? 0,
              "answered_questions": ans.answeredquestions
          ] as [String : Any]
...
//                let anser : AnsweredQuestions = AnsweredQuestions(map: <#T##Map#>)
//                answers?.answeredquestions.append(anser)
                tableView.reloadData()
        }
}
